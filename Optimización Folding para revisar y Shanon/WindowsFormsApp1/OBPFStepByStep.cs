...
namespace OptimizacionFolding
{
    ...
    public partial class Form1 : Form
    {
        ...
        public void sintesis(ref datos_de_corrida datos)
        {
            for (int i = 1; i < MAX_PROTEINAS; i++)
            {
                datos.proteinas[i].nombre_proteina = "datos.proteinas " + i.ToString();
                datos.proteinas[i].aminoacidos_actual = new double[MAX_AMINOACIDOS];
                datos.proteinas[i].aminoacidos_reserva = new double[MAX_AMINOACIDOS];
                datos.proteinas[i].aminoacidos_mejor = new double[MAX_AMINOACIDOS];
                datos.proteinas[i].SI = new tipos_aminoacidos[MAX_AMINOACIDOS];
        ...
        private void operador_proteostasis(ref datos_de_corrida datos)
        {
            ...
            for (int i = 1; i <= datos.cantidad_de_proteinas; i++)
            {
                datos.proteinas[i].valor_funcion_actual = datos.f.Function(datos.proteinas[i].aminoacidos_actual, datos.dimensiones, datos.funcion);
                datos.proteinas[i].valor_funcion_reserva = datos.proteinas[i].valor_funcion_actual;
                for (int k = 1; k <= datos.dimensiones; k++)
                    datos.proteinas[i].aminoacidos_reserva[k] = datos.proteinas[i].aminoacidos_actual[k];
                if (datos.proteinas[i].valor_funcion_actual < datos.proteinas[i].mejor_valor_de_la_funcion)
                {
                    datos.proteinas[i].mejor_valor_de_la_funcion = datos.proteinas[i].valor_funcion_actual;
                    for (int k = 1; k <= datos.dimensiones; k++)
                        datos.proteinas[i].aminoacidos_mejor[k] = datos.proteinas[i].aminoacidos_actual[k];
                    if (datos.proteinas[i].mejor_valor_de_la_funcion < datos.mejor_valor_global)
                    {
                        datos.mejor_valor_global = datos.proteinas[i].valor_funcion_actual;
                        if (datos.mejor_valor_global < System.Convert.ToDouble(textBox23.Text))
                        {
                            textBox23.Text = datos.mejor_valor_global.ToString();
                            textBox23.Refresh();
                        ...
                        for (int k = 1; k <= datos.dimensiones; k++)
                            datos.mejor_posicion_global[k] = datos.proteinas[i].aminoacidos_actual[k];
                    }
                }
            }
            QuickSort_vector_proteinas(ref datos.proteinas, 1, datos.cantidad_de_proteinas);
            for (int i = 1; i <= datos.cantidad_de_proteinas; i++)
            {
                ...
                datos.proteinas[i].hay_chaperona = false;
                datos.proteinas[i].chaperonas.Clear();
                datos.proteinas[i].hay_cuaternarias = false;
                datos.proteinas[i].cuaternarias.Clear();
                datos.proteinas[i].quienes_son_HI.Clear();
                datos.proteinas[i].quienes_son_HS.Clear();
                datos.proteinas[i].quienes_son_PI.Clear();
                datos.proteinas[i].quienes_son_PS.Clear();
                for (int aminoacido = 1; aminoacido <= datos.dimensiones; aminoacido++)
                {
                    datos.proteinas[i].densidad_local[aminoacido] = calcula_Ro(ref datos, i, aminoacido);
                    clasifica_aminoacidos(ref datos, i, aminoacido);
                }
            }
#if IMPRIME_CODIGO_INTERMEDIO
        ...
        void lee_datos_del_clipboard_para_ejecutar(ref datos_de_corrida datos)
        {
        ...
        void procedimiento_de_ejecucion_individual(ref datos_de_corrida datos)
        {
            ...
            lee_datos_del_clipboard_para_ejecutar(ref datos);
            datos.mejor_valor_global = double.MaxValue;
#if POSIBILIDAD_DE_GRAFICAR
        ...
        private void copiarParámetrosAlClipboardToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string cadena = "";
